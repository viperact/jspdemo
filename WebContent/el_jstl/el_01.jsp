...
<h3>주석처리</h3>
	...
	<%--
	 ...
	 <% 
	 	...
	 	// page(영역) - pageContext(jsp에서 제공하는 기본객체),
	 	// request - request(내부객체),
	 	// session - session,
	 	// application - application
	 	
	 	//page영역에 p1이름으로 page값이 저장된다.
	 	pageContext.setAttribute("p1", "page"); // 한페이지 (el_01.jsp에서만 참조가능)
	 	...
	 	application.setAttribute("p4", "application"); // 모든 클라이언트들이 공유하는 목적(방문횟수 등)
	 %>
	 <p>
	 ...
	 <p>
	 <!-- 
	 	속성값을 가져올 때 영역을 명시하지 않으면
	 	page -> request -> session -> application순으로 검색을 한다.
	  -->
	 ${p3}
	 </p>
</body>
